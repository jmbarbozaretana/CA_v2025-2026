...
def estimateThresholds(pts,total_pts,bw,n_components_range):
    ...
    thres10=0.05  ## Umbral de decisión para el abandono
...
def thresholdStats(pts,
    ...
    for bw in bws:
        for _ in range(reps):
            line=list(estimateThresholds(pts,total_pts,bw,n_components_range))
            line.append(bw)
            line=np.array(line)
